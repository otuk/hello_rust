...
the chapter_00/mod.rsfile we are in. The directory chapter_00 is our 
_chapter 0_ module directory. Cargo.lock is not to be hand edited, it reflects 
the status of Cargo.toml file contents at a certain point in time. More on this later.
...
To run this project all we need to do is execute ```cargo run``` from the 
project root hello_rust directory.
